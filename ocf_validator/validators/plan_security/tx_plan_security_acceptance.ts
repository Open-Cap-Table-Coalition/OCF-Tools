const valid_tx_plan_security_acceptance = (context: any, event: any, isGuard: Boolean) => {
  const {transactions} = context.ocfPackageContent;

  let validity = false;
  let report: any = {transaction_type: "TX_PLAN_SECURITY_ACCEPTANCE", transaction_id: event.data.id, transaction_date: event.data.date};
  ...
  // Check that plan security issuance in incoming security_id does not have a plan security retraction transaction associated with it.
  let no_plan_security_retraction_validity = false;
  let plan_security_retraction_exists = false;
  transactions.forEach((ele: any) => {
    if (
      ele.security_id === event.data.security_id &&
      ele.object_type === 'TX_PLAN_SECURITY_RETRACTION'
    ) {
      plan_security_retraction_exists = true;
    }
  });

  if (!plan_security_retraction_exists) {
    no_plan_security_retraction_validity = true;
    report.no_plan_security_retraction_validity = true;
  }
  if (!no_plan_security_retraction_validity) {
    report.no_plan_security_retraction_validity = false;
  }

  if (
    incoming_planSecurityIssuance_validity &&
    incoming_date_validity &&
    no_plan_security_retraction_validity
  ) {
    validity = true;
  }
